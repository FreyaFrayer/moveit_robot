...
def generate_launch_description() -> LaunchDescription:
    ...
    moveit_cpp_yaml = os.path.join(
        get_package_share_directory("panda_ik_benchmark"),
        "config",
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument("num_points", default_value="3"),
            ...
            DeclareLaunchArgument("data_root", default_value="data"),
            ...
            DeclareLaunchArgument("ws_x_min", default_value="-0.75"), # 0.15
            ...
            Node(
                package="panda_ik_benchmark",
                executable="ik_benchmark",
                output="screen",
                ...
                arguments=[
                    ...
                    "--num-spaces", num_spaces,
                    "--max-attempts", max_attempts,
                    "--ik-timeout", ik_timeout,
                    "--resample-max", resample_max,
                    "--topup-passes", topup_passes,
                    "--precheck-attempts", precheck_attempts,
                    "--precheck-num-spaces", precheck_num_spaces,
                    "--ws-x", ws_x_min, ws_x_max,
                    "--ws-y", ws_y_min, ws_y_max,
                    "--ws-z", ws_z_min, ws_z_max,
                    "--min-sep", min_sep,
                    "--time-model", time_model,
                    "--totg-vel-scale", totg_vel_scale,
                    "--totg-acc-scale", totg_acc_scale,
                    "--totg-path-tolerance", totg_path_tolerance,
                    "--totg-resample-dt", totg_resample_dt,
                    "--totg-min-angle-change", totg_min_angle_change,
                ],
            ),
        ]
    )
